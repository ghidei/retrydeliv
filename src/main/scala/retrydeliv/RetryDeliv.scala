...
case class Acknowledgement(pload: String)
...
object Node {
  def props(retryAttempts: Int): Props = Props(new Node(retryAttempts))
}

class Node(retryAttempts: Int) extends Actor {
  val log: LoggingAdapter = Logging(context.system, this)
  val name: String = self.path.name
  implicit val timeout: Timeout = Timeout(1.seconds)
  def receive: Receive = {
    case bcast @ Broadcast(pload) =>
      //log.info(s"Actor $name received $bcast")
      //Log the payload in global logs
      logBroadcast(pload)
      sender() ! Acknowledgement(pload)
    case start @ Start(Broadcast(pload)) =>
      //log.info(s"Actor $name received $start")
  ...
  def sendBroadcast(broadcast: Broadcast): Unit = {
    Relations.relations.get(name) match {
      ...
      case Some(neighbors: List[ActorRef]) =>
        for(neigh <- neighbors) {
          sendMessage(neigh, broadcast, retryAttempts)
        }
  ...
  def sendMessage(neigh: ActorRef, broadcast: Broadcast, attemptsLeft: Int): Unit = {
    /*
    val neighborName = neigh.path.name
    log.info(s"Actor $name sending $broadcast to $neighborName")
    */
    val ackFuture = neigh ? broadcast
    val ack = Await.ready(ackFuture, Duration.Inf).value.get
    val result = ack match {
      case Success(t) =>
        //log.info(s"Actor $name succeeded when sending $broadcast to actor $neighborName")
        true
      case Failure(e) => false
    }
    if(!result && attemptsLeft > 0){
      //log.info(s"Actor $name failed when sending $broadcast to $neighborName")
      sendMessage(neigh, broadcast, attemptsLeft - 1)
    }
  }
...
class RetryDeliv {
  ...
  //Creating nodes
  val A: ActorRef = system.actorOf(Node.props(1), "A")
  val B: ActorRef = system.actorOf(Node.props(1), "B")
  val C: ActorRef = system.actorOf(Node.props(1), "C")
